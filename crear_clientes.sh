#!/bin/bash
##############################################################################
# Script: crear_clientes.sh
# Descripción: Crea sitios web y usuarios FTP a partir de un archivo CSV
# Autor: Juanjo Gomez
# Fecha: 03-31-2026
# Uso: sudo ./crear_clientes.sh [ruta_al_csv]
##############################################################################
...
CSV_FILE="${1:-/home/axisadmin/clientes/clientes.csv}"  # Primer argumento o ruta por defecto
...
# Función para comprobar que Apache y FTP están instalados
comprobar_servicios() {
    if ! command -v apache2 &> /dev/null; then
        mensaje "ERROR" "ERROR" "Apache2 no está instalado"
        log "ERROR" "Apache2 no está instalado"
        exit 1
    fi
    
    if ! command -v vsftpd &> /dev/null; then
        mensaje "ERROR" "ERROR" "vsftpd no está instalado"
        log "ERROR" "vsftpd no está instalado"
        exit 1
    fi
    
    mensaje "EXITOSO" "OK" "Servicios verificados correctamente"
    log "INFO" "Servicios verificados: Apache2 y vsftpd presentes"
}
...
crear_usuario_ftp() {
    ...
    # Crear usuario sin shell interactiva, con home específico
...
configurar_permisos() {
    ...
    # Permisos: usuario rwx, grupo rx, otros sin acceso
    ...
    mensaje "EXITOSO" "OK" "Permisos configurados para: $dominio"
...
EOF
    ...
    mensaje "EXITOSO" "OK" "Index.html creado para: $dominio"
...
EOF
    ...
    mensaje "EXITOSO" "OK" "VirtualHost creado y habilitado: $dominio"
...
proceso_crear_cliente() {
    ...
    mensaje "EXITOSO" "SUCCESS" "Cliente $dominio procesado correctamente"
...
recargar_apache() {
    ...
    mensaje "EXITOSO" "OK" "Apache recargado correctamente"
...
mostrar_resumen() {
    ...
    mensaje "EXITOSO" "RESUMEN" "Exitosos: $success"
    if [[ $failed -gt 0 ]]; then
